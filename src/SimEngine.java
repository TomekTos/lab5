public class SimEngine
{
    ...
    public SimEngine(double mm, double kk, double cc, double LL0, double gg,Vector2D yy, Vector2D vv, Vector2D yy00)
    {
        ...
        else masa = mm; wsp_spr = kk; wsp_tlum = cc; dlug_swob = LL0; grawitacja = gg; pol_masy = yy; pred_masy = vv;pkt_zaw = yy00;
    }
    public double dajMase()
    {
        return masa;
    }
    public double dajWsp_spr()
    {
        return wsp_spr;
    }
    public double dajWsp_tlum()
    {
        return wsp_tlum;
    }
    public double dajDlug_swob()
    {
        return dlug_swob;
    }
    ...
    {
        if (wspSpr < 0) wsp_spr = 0; else wsp_spr = wspSpr;
    }
    void ustawDlug_swob(double dlugSwob)
    {
        if (dlugSwob < 0) dlug_swob = 0;
    ...
    public void liczenieWektorow(double timestep)
    {
        ...
        silaSprezystosci = pkt_zaw.subVectors(dajPol_masy()).normalVector().mulVector(wsp_spr*(pol_masy.subVectors(dajPkt_zaw()).length()- dajDlug_swob()));
        silaTlumienia = pred_masy.mulVector(-wsp_tlum);
        Vector2D silaWypadkowa = new Vector2D();
        silaWypadkowa = silaSprezystosci.addVectors(silaTlumienia).addVectors(silaGrawitacji);
